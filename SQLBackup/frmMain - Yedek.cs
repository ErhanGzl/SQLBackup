...
namespace SQLBackup
{
    public partial class frmSQLYedeklemeYedek : Form
    {
        public frmSQLYedeklemeYedek()
        {
            InitializeComponent();
        }
        ...
        bool FormYuklemesiTamamlandi = false;
        public bool DilDegistir = false;
        public string DegistirilenDil = string.Empty;
        ...
        private void frmSQLYedekleme_Load(object sender, EventArgs e)
        {
            ...
            linkLabel_Katilim.Text = "Uygulama Hakkında";
        ...
        private void timer_Zamanlayici_Tick(object sender, EventArgs e)
        {
            ...
            lbl_KalanSure.Text = (PlanlananSaat).ToString("h'sa 'm'dk 's's'");
        }
        ...
        {
            if (FormYuklemesiTamamlandi)
            {
                Cikis = true;

                ProcessStartInfo startInfo = Process.GetCurrentProcess().StartInfo;
                startInfo.FileName = Application.ExecutablePath;
                startInfo.Arguments = cmb_UygulamaDili.Text;
        ...
        private void Baslat()
        {
            ...
            if (AyarlariOku() != null)
            {
                ...
                cmb_Database.SelectedItem = ayarlar.Veritabani;
                ...
                if (DilDegistir)
                    cmb_UygulamaDili.Text = DegistirilenDil;
                else
        ...
        {
            if (chk_WindowsAuthentication.Checked)
                txt_Connection.Text = string.Format("data source ='{0}';Initial Catalog = {1}; Integrated Security = SSPI; ", cmb_Server.Text, cmb_Database.Text);
            else
                txt_Connection.Text = string.Format("data source ={0};Initial Catalog = {1}; User Id={2};Password='{3}';", cmb_Server.Text, cmb_Database.Text, txt_KullaniciAdi.Text, txt_Sifre.Text);
        }
        ...
        private void SQLYedekle()
        {
            try
            {
                string BackupFileName = DateTime.Now.ToString("ddMMyyyyHHmms") + ".bak";
                SqlConnection con = new SqlConnection(txt_Connection.Text);
                SqlCommand cmd = new SqlCommand();
                cmd.Connection = con;
                cmd.CommandText = "BACKUP DATABASE " + cmb_Database.Text + " TO DISK = '" + ayarlar.SQLYedeklemeKlasoru + "\\" + BackupFileName + "';";
    ...
    /// </summary>
    public class SQLBackupAyarlarYedek
    {
